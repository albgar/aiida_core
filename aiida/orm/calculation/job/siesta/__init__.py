...
class SiestaCalculation(JobCalculation):
    ...
    Add docs
    """
    _siesta_plugin_version = '0.7.BosoniE'
    ...
    def _prepare_for_submission(self,tempfolder,
        ...
        # bandskpoints can be undefined, meaning we don't want to calculate bands.
	# In this case we do nothing: we don't add any block related to BandsLines/Points.
        # An internal flag will help us to process the two cases
        ...
        # --------------- K-POINTS ----------------!
        #only mesh is implemented, isn't it???? And gamma_only
        if True:   #What's this?? if True???
            try:
	...
	# If flagbands=True we also add the bands file to the retrive list!
	# This is extremely important because the parser parses the bands
	# only if aiida.bands is in the retrive list!!
